...
ArgVector::ArgVector(char *pString, std::size_t stringLength) { resolveStringToArgs(pString, stringLength); }
...
bool ArgVector::resolveStringToArgs(char *pString, std::size_t stringLength) {
    ...
    if (pString && stringLength) {
        ...
        for (char *pStringHead = pString; static_cast<std::size_t>(pStringHead - pString) < stringLength;
             ++pStringHead) {
